...
export async function POST(req: Request) {
  try {
    ...
    const approvedIds = new Set(
      initial.filter((s) => s.status === 'approved').map((s) => s.id)
    );
    ...
    if (existing) {
      if (sameCanonicalBallot(existing.submissionIds, requested)) {
        return NextResponse.json({
          ok: true,
          recorded: existing.submissionIds.length,
    ...
    if (final.length === 0) {
      return NextResponse.json(
        { ok: false, error: 'selection-unavailable' },
        { status: 409 }
      );
    }

    const created = await writeBallotOnce(event.slug, voter, {
      submissionIds: final,
      submittedAt: new Date().toISOString(),
    });
    if (created === 'exists') {
      const latest = await getBallot(event.slug, voter);
      if (latest && sameCanonicalBallot(latest.submissionIds, requested)) {
        return NextResponse.json({
          ok: true,
          recorded: latest.submissionIds.length,
          alreadyRecorded: true,
        });
      }
      return NextResponse.json(
